...
export class FunctionalUnit {
    ...
    public tic() {
        if (!this.isStalled()) {
            ...
            this._currentBlankTimeUnits = Math.min(this._currentBlankTimeUnits, this._latency - 1);  // it cannot be more than the latency
        } else {
            // decrease stall time
            this._stalled--;
        }
    ...
    public executeReadyInstruction(firstValue: number = 0, secondValue: number = 0): FunctionalUnitResult {
        if (this._instructions.length > 0 && this._instructions[0].blankTimeUnitsAhead == 0) {
    ...
    public addInstruction(instruction: Instruction): number {
        ...
        let blankTimeUnitsAhead = (this._currentBlankTimeUnits > 0) ? this._currentBlankTimeUnits : 0;
        this._instructions.push({ instruction: instruction, ref: ref, blankTimeUnitsAhead: blankTimeUnitsAhead });
