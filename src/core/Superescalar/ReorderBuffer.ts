...
export class ReorderBuffer {
	...
	public canCommitStoreInstruction(): boolean {
		...
		return entry?.ready && entry.instruction.isStoreInstruction();
	}
	...
	public canCommitJumpInstruction(): boolean {
		const entry = this._queue[0];
		return entry?.ready && entry.instruction.isJumpInstruction();
	}
	...
	public canCommitRegisterInstruction(): boolean {
		const entry = this._queue[0];
		return entry?.ready && entry.instruction.isRegisterInstruction();
	}
