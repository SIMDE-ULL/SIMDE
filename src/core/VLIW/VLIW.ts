...
export class VLIW extends Machine {
	...
	public checkCode() {
		for (let i = 0; i < this._code.getLargeInstructionNumber(); i++) {
			...
			for (let j = 0; j < instruction.getVLIWOperationsNumber(); j++) {
				...
				if (
					operation.getFunctionalUnitIndex() >=
					this.functionalUnit[operation.getFunctionalUnitType()].length
				) {
	...
	public tic() {
		...
		for (i = 0; i < instruction.getVLIWOperationsNumber(); i++) {
			...
			if (
				!this.functionalUnit[type][index].isFree() ||
		...
		for (i = 0; i < instruction.getVLIWOperationsNumber(); i++) {
			...
			this.functionalUnit[operation.getFunctionalUnitType()][
				operation.getFunctionalUnitIndex()
		...
		switch (operation.opcode) {
			case Opcodes.ADD:
				this._gpr.setContent(
					...
					this._gpr.content[operation.getOperand(1)] +
						this._gpr.content[operation.getOperand(2)],
					true,
				);
				break;
			case Opcodes.MULT:
			...
			case Opcodes.ADDI:
				this._gpr.setContent(
					operation.getOperand(0),
					this._gpr.content[operation.getOperand(1)] + operation.getOperand(2),
					true,
				);
				break;
			case Opcodes.ADDF:
				this._fpr.setContent(
					operation.getOperand(0),
					this._fpr.content[operation.getOperand(1)] +
						this._fpr.content[operation.getOperand(2)],
					true,
				);
				break;
			case Opcodes.MULTF:
				...
				break;
			case Opcodes.SW:
				this._memory.setDatum(
					this._gpr.content[operation.getOperand(2)] + operation.getOperand(1),
					this._gpr.content[operation.getOperand(0)],
				);
				break;
			case Opcodes.SF:
				this._memory.setDatum(
					this._gpr.content[operation.getOperand(2)] + operation.getOperand(1),
					this._fpr.content[operation.getOperand(0)],
				);
				break;
			case Opcodes.LW: {
				const datumInteger = this._memory.getFaultyDatum(
					this._gpr.content[operation.getOperand(2)] + operation.getOperand(1),
				);
			...
			case Opcodes.LF: {
				const datumFloat = this._memory.getFaultyDatum(
					this._gpr.content[operation.getOperand(2)] + operation.getOperand(1),
				);
	...
	private checkDependencies(_row: number, _id: number) {
		...
		for (let row = 0; row < this._code.getLargeInstructionNumber(); row++) {
			...
			for (let j = 0; j < instruction.getVLIWOperationsNumber(); j++) {
				if (
					!checkSourceOperands(instruction.getOperation(j), checkGPR, checkFPR)
				) {
			...
			for (let j = 0; j < instruction.getVLIWOperationsNumber(); j++) {
				if (instruction.getOperation(j).getPred() !== 0) {
					for (index = 0; index < controlCheckList.length; index++) {
						if (
							instruction.getOperation(j).getPred() ===
							controlCheckList[index].register
						) {
							break;
						}
					}
					...
					if (
						this.functionalUnit[
							instruction.getOperation(j).getFunctionalUnitType()
						].length < controlCheckList[index].latency
					) {
						const id = instruction.getOperation(j).id;
						for (let i = 0; i < controlCheckList.length; i++) {
							controlCheckList[i].latency = 0;
