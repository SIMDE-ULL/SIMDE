import {
  ...
  type FunctionalUnit,
  type FunctionalUnitResult,
  FunctionalUnitType,
} from "../Common/FunctionalUnit";
import { Machine } from "../Common/Machine";
import { Opcodes } from "../Common/Opcodes";
import { type Check, DependencyChecker } from "./DependencyChecker";
...
export class VLIW extends Machine {
  ...
  public tic() {
      ...
      for (j = 0; j < this.functionalUnit[i].length; j++) {
        ...
        if (instToExecute && this._predR[instToExecute.getPred()]) {
          const registerFile = instToExecute.isDestinyRegisterFloat()
            ...
            : this._gpr;
          const firstOperand =
            registerFile.content[instToExecute.getFirstOperandRegister()];
          const secondOperand = instToExecute.hasImmediateOperand()
            ? instToExecute.getImmediateOperand()
            : registerFile.content[instToExecute.getSecondOperandRegister()];
    ...
    for (i = 0; i < instruction.getVLIWOperationsNumber(); i++) {
      ...
      if (
        ...
        DependencyChecker.checkNat(
          instruction.getOperation(i),
          this._NaTGP,
          this._NaTFP,
        )
      ) {
  ...
  private runOperation(
    execution: FunctionalUnitResult,
  ...
  private checkDependencies(row: number, _id: number) {
    ...
    for (let r = 0; r < this._code.getLargeInstructionNumber(); r++) {
      ...
      for (let j = 0; j < instruction.getVLIWOperationsNumber(); j++) {
        if (
          !DependencyChecker.checkSourceOperands(
            instruction.getOperation(j),
            checkGPR,
            checkFPR,
          )
        ) {
    ...
    for (let r = 0; r < this._code.getLargeInstructionNumber(); r++) {
      ...
      for (let j = 0; j < instruction.getVLIWOperationsNumber(); j++) {
        if (
          ...
          const check1: Check = { latency: 0, register: 0 };
          const check2: Check = { latency: 0, register: 0 };
