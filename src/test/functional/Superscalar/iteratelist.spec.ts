...
const context = {} as { code: Code; machine: Superscalar };
...
test("recorrelista.pla is executed properly", () => {
  ...
  for (let i = 0; i < memContent.length; i++) {
    context.machine.memory.setData(memContentBaseAddress + i, memContent[i]);
  }
  ...
  while (context.machine.tic() !== SuperscalarStatus.SUPER_ENDEXE) {
    if (context.machine.status.cycle === 40) {
      ...
      expect(
        context.machine.functionalUnit[FunctionalUnitType.FLOATINGMULTIPLY][0]
          .getVisualData()
          .filter((e) => e.id === 13).length,
      ).toBeGreaterThan(0); // Instruction 13 is not at FLOATINGMULTIPLY Functional unit at cycle 40');
      expect(
        context.machine.functionalUnit[FunctionalUnitType.MEMORY][0]
          .getVisualData()
          .filter((e) => e.id === 16).length,
      ).toBeGreaterThan(0); // Instruction 16 is not at MEMORY Functional unit at cycle 40');
      expect(
        context.machine.functionalUnit[FunctionalUnitType.JUMP][0]
          .getVisualData()
          .filter((e) => e.id === 12).length,
      ).toBeGreaterThan(0); // Instruction 12 is not at JUMP Functional unit at cycle 40');
    }

    if (
      context.machine.status.cycle === 60 ||
      context.machine.status.cycle === 70
    ) {
      // at cycle 70 the jump is still looping
