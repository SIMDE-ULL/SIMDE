...
test("Bucle.pla is executed properly", (t) => {
  ...
  const result = Array.from(context.machine.memory)
    .map((d) => d.value)
    .slice(resultBaseAddress, resultBaseAddress + resultContent.length);
...
test("Bucledoble.pla is executed properly", (t) => {
  ...
  const result = Array.from(context.machine.memory)
    .map((d) => d.value)
    .slice(resultBaseAddress, resultBaseAddress + 5);
  expect(result).toStrictEqual(
    resultContent.map((x) => x * sumContent).slice(0, 5),
  );
});
