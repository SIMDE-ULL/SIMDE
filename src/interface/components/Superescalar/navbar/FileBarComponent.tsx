...
class FileBarComponent extends React.Component<any, any> {
	...
	downloadContentFile() {
		if (SuperescalarIntegration.contentIntegration) {
			downloadTextFile(
				"content.txt",
				SuperescalarIntegration.contentIntegration.deparse(),
			);
		} else {
	...
	downloadCodeFile() {
		if (SuperescalarIntegration.superescalar.code) {
			downloadTextFile(
				"code.txt",
				SuperescalarIntegration.superescalar.code.save(),
			);
		} else {
			...
			<Dropdown className="smd-filebar">
				...
				>
					...
					<Dropdown.Item
						...
						onClick={() => {
							downloadJsonFile(
								"stats.json",
								SuperescalarIntegration.stats.exportStats(),
							);
						}}
					>
				...
				>
					<Dropdown.Item
						eventKey="1"
						onClick={() => {
							this.props.actions.toggleSuperConfigModal(true);
						}}
					>
					...
					<Dropdown.Item
						eventKey="1"
						onClick={() => {
							this.props.actions.toggleSuperescalarLoadContentModal(true);
						}}
					>
...
export default connect(
	mapStateToProps,
	mapDispatchToProps,
)(withTranslation()(FileBarComponent));
